...
#ifdef CONFIG_ARGOS_SYSFS
static ssize_t store_threshhold_sys(struct boost_table *t, const char *buf, size_t size)
{
	unsigned int value;
	int err;
	...
	if (err)
		return err;
	t->items[THRESHOLD] = value;
	return size;
}
static ssize_t store_cpu_min_freq_sys(struct boost_table *t, const char *buf, size_t size)
{
	unsigned int value;
	int err;
	...
	t->items[CPU_MIN_FREQ] = value;
	return size;
}
static ssize_t store_cpu_max_freq_sys(struct boost_table *t, const char *buf, size_t size)
{
	unsigned int value;
	int err;
	...
	t->items[CPU_MAX_FREQ] = value;
	return size;
}
static ssize_t store_kfc_min_freq_sys(struct boost_table *t, const char *buf, size_t size)
{
	unsigned int value;
	int err;
	...
	t->items[KFC_MIN_FREQ] = value;
	return size;
}
static ssize_t store_kfc_max_freq_sys(struct boost_table *t, const char *buf, size_t size)
{
	unsigned int value;
	int err;
	...
	t->items[KFC_MAX_FREQ] = value;
	return size;
}
static ssize_t store_mif_freq_sys(struct boost_table *t, const char *buf, size_t size)
{
	unsigned int value;
	int err;
	...
	t->items[MIFFREQ] = value;
	return size;
}
static ssize_t store_int_freq_sys(struct boost_table *t, const char *buf, size_t size)
{
	unsigned int value;
	int err;
	...
	t->items[INTFREQ] = value;
	return size;
}
static ssize_t store_task_affinity_sys(struct boost_table *t, const char *buf, size_t size)
{
	unsigned int value;
	int err;
	...
	t->items[TASK_AFFINITY_EN] = value;
	return size;
}
static ssize_t store_irq_affinity_sys(struct boost_table *t, const char *buf, size_t size)
{
	unsigned int value;
	int err;
	...
	t->items[IRQ_AFFINITY_EN] = value;
	return size;
}

static ssize_t show_threshhold_sys(struct boost_table *t, char *buf)
{
	return sprintf(buf, "%u Mbps\n", t->items[THRESHOLD]);
}
static ssize_t show_cpu_min_freq_sys(struct boost_table *t, char *buf)
{
	return sprintf(buf, "%u Hz\n", t->items[CPU_MIN_FREQ]);
}
static ssize_t show_cpu_max_freq_sys(struct boost_table *t, char *buf)
{
	return sprintf(buf, "%u Hz\n", t->items[CPU_MAX_FREQ]);
}
static ssize_t show_kfc_min_freq_sys(struct boost_table *t, char *buf)
{
	return sprintf(buf, "%u Hz\n", t->items[KFC_MIN_FREQ]);
}
static ssize_t show_kfc_max_freq_sys(struct boost_table *t, char *buf)
{
	return sprintf(buf, "%u Hz\n", t->items[KFC_MAX_FREQ]);
}
static ssize_t show_mif_freq_sys(struct boost_table *t, char *buf)
{
	return sprintf(buf, "%u Hz\n", t->items[MIFFREQ]);
}
static ssize_t show_int_freq_sys(struct boost_table *t, char *buf)
{
	return sprintf(buf, "%u Hz\n", t->items[INTFREQ]);
}
static ssize_t show_task_affinity_sys(struct boost_table *t, char *buf)
{
	return sprintf(buf, "%s\n", t->items[TASK_AFFINITY_EN] ? "On" : "Off");
}
static ssize_t show_irq_affinity_sys(struct boost_table *t, char *buf)
{
	return sprintf(buf, "%s\n", t->items[IRQ_AFFINITY_EN] ? "On" : "Off");
}
struct argos_attr {
	struct attribute attr;
	ssize_t (*show)(struct boost_table *, char *);
	ssize_t (*store)(struct boost_table *, const char *, size_t);
};
#define argos_sys_attr_rw(_name)			    \
	static struct argos_attr _name ## _argos_sys =	    \
		__ATTR(_name, 0660, show_ ## _name ## _sys, \
		       store_ ## _name ## _sys)

argos_sys_attr_rw(threshhold);
argos_sys_attr_rw(cpu_min_freq);
argos_sys_attr_rw(cpu_max_freq);
argos_sys_attr_rw(kfc_min_freq);
argos_sys_attr_rw(kfc_max_freq);
argos_sys_attr_rw(mif_freq);
argos_sys_attr_rw(int_freq);
argos_sys_attr_rw(task_affinity);
argos_sys_attr_rw(irq_affinity);

static struct attribute *argos_attributes[] = {
	&threshhold_argos_sys.attr,
	&cpu_min_freq_argos_sys.attr,
	&cpu_max_freq_argos_sys.attr,
	&kfc_min_freq_argos_sys.attr,
	&kfc_max_freq_argos_sys.attr,
	&mif_freq_argos_sys.attr,
	&int_freq_argos_sys.attr,
	&task_affinity_argos_sys.attr,
	&irq_affinity_argos_sys.attr,
	NULL,
};

#define to_table(k) container_of(k, struct boost_table, kobj)
#define to_attr(a)  container_of(a, struct argos_attr, attr)

static ssize_t show(struct kobject *kobj, struct attribute *attr, char *buf)
{
	struct boost_table *t = to_table(kobj);
	struct argos_attr *aattr = to_attr(attr);
	ssize_t ret = -EINVAL;

	if (aattr->show)
		ret = aattr->show(t, buf);
	else
		ret = -EIO;
...
static inline void argos_free_sysfs_kobj(struct boost_table *t)
{
	kobject_put(&t->kobj);
	wait_for_completion(&t->kobj_unregister);
}

static void argos_sysfs_release(struct kobject *kobj)
{
	struct boost_table *t = to_table(kobj);

	pr_err("%s: release it\n", __func__);

	complete(&t->kobj_unregister);
}

static struct kobj_type ktype_argos = {
	.sysfs_ops = &sysfs_ops,
	.default_attrs = argos_attributes,
	.release = argos_sysfs_release,
};
#endif
...
#ifdef CONFIG_ARGOS_SYSFS
static int argos_make_sysfs(struct device *dev, struct argos *sdev, int count)
{
	int i;
	int ret = -EINVAL;

	for (i = 0; i < count; i++) {
		init_completion(&sdev->tables[i].kobj_unregister);
		ret = kobject_init_and_add(&sdev->tables[i].kobj, &ktype_argos,
					   &dev->kobj, "%s_level_%d", sdev->desc, i);
		if (ret) {
			pr_err("kojb init and add failed : %d\n", ret);
			goto err_sysfs;
		}
		kobject_uevent(&sdev->tables[i].kobj, KOBJ_ADD);
	}
	return ret;
err_sysfs:
	for (i = i - 1; i >= 0; i--)
		argos_free_sysfs_kobj(&sdev->tables[i]);
	return ret;
}
#endif
...
#ifdef CONFIG_ARGOS_SYSFS
		retval = argos_make_sysfs(dev, sdev, num_level);
		if (retval) {
			dev_err(dev, "Failed to make sysfs node\n");
			goto err_out;
		}
#endif
		device_count++;
